...
BEGIN
  ...
  -- Só registra quando houver intenção clara de agenda/consulta/reunião/atendimento confirmada ou marcada.
  IF t !~ '(agend|marc|consulta|reuni[aã]o|atendimento|hor[aá]rio|confirmad)' THEN
    RETURN NEW;
  END IF;

  -- Hora: 14:30, 14h30, às 14h, as 14, horário 14.
  m := regexp_match(t, '(?:[aà]s|as|hor[aá]rio\s*)?\s*(\d{1,2})(?:[:h](\d{2}))\s*(?:h|hs|horas)?\b');
  IF m IS NOT NULL THEN
    hh := m[1]::int; mm := coalesce(m[2], '0')::int;
  ELSE
    m := regexp_match(t, '(?:[aà]s|as|hor[aá]rio\s*)\s*(\d{1,2})\s*(?:h|hs|horas)?\b|\b(\d{1,2})\s*(?:h|hs|horas)\b');
    IF m IS NOT NULL THEN
      hh := coalesce(m[1], m[2])::int; mm := 0;
    ELSE
      RETURN NEW;
    END IF;
  END IF;
  ...
  -- Data: hoje, amanhã, DD/MM, DD/MM/AAAA.
