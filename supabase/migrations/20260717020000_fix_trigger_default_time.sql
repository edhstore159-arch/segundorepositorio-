...
-- Quando o cliente envia mensagem de agendamento SEM horário
-- (ex: "dia 30", "agendar para amanhã"), usa horário padrão
-- 09:00 ao invés de descartar silenciosamente o agendamento.
-- ============================================================
...
BEGIN
  ...
  -- ============================================================
  -- FIX: Se nao achou horario mas tem data, usar 09:00 como
  -- padrao ao inves de descartar o agendamento.
  -- ============================================================
  IF hh IS NULL OR hh < 0 OR hh > 23 OR mm < 0 OR mm > 59 THEN
    -- Se NÃO é confirmação e NÃO é reagendamento, usa horário padrão
    IF NOT is_confirmation AND NOT is_reschedule THEN
      hh := 9; mm := 0;
    ELSE
      RETURN NEW;
    END IF;
